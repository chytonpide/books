...
clox 에서는 파싱과 컴파일링을 분리하지만 단순함을 위해서 두가지 패스를 하나로하는 컴파일러를 구현한다.
이렇게 하지않으면 c 에서는 전체 파싱 결과를 기억하기 위해서는 이를 위한 많은 추가적인 코드를 작성해야한다.
lox 는 적은 맥락인 두개의 토큰 만으로 구문 문법을 이해할 수 있도록 설계되어있다.
스캔을 통해서 최근 두개의 토큰을 읽고 저장하기 위한 Parser 구조체와 유틸 함수 advance 를 구현한다.
바이트코드를 쓰는 유틸 함수 emitByte 를 구현한다.

일단은 숫자, 그루핑, 단항부정(unary negation), 사칙연산을 컴파일 해본다.
먼저 prefix expression 을 위한 파싱 함수를 만든다. 
prefix expression 은 연산자가 피 연산자 앞에 나오는 표현식으로 특정 토큰으로 시작하는 것으로 그 표현식을 알아 챌 수 있다.
예들들어 grouping 과 unary negation 이 그러하다.
`-a.b + c;` 와 같은 표현식을 파싱할 때, clox 에서는 parsePrecedence 함수를 사용해서 우선순위 문제를 해결한다.
parsePrecedence 는 인수로 넘어온 Precedence 이상에 해당하는 표현식만을 파싱한다.

infix expression 을 위한 파싱 함수를 만든다.
infix expression 은 연산자가 피연산자 사이에 위치하는 표현식으로, 왼쪽 피연산자를 파싱한 후에 연산자 토큰을 만날때 비로서 그 표현식을 알아 챌 수 있다.
binary 함수는 중위식을 파싱하는 함수중 하나이다. 중위식 오른쪽 피연산자 또한 파싱 해야한다.
이때 올바른 파싱을 위해서는 우측 피연산자를 올바른 우선순위에 맞게 파싱해야 한다.
`1 + 2 * 3` 와 같은 표현식을 파싱할 때 `+` 의 우측 피연산자는 `2`가 아니라 `2 * 3` 이 파싱되어야 한다.     

결국 Pratt Parser 에서는 파싱을 하기 위해서 다음과 같은 정보를 찾을 수 있는 테이블이 필요한다.
 - 해당 타입의 토큰으로 시작하는 접두(prefix) 표현식을 컴파일하는 함수
 - 왼쪽 피연산자 뒤에 해당 타입의 토큰이 오는 중위(infix) 표현식을 컴파일하는 함수
 - 해당 토큰을 연산자로 사용하는 중위 표현식의 연산자 우선순위
이를 ParseRule 이라는 타입으로 표현하고, 이 정보를 사용해서 우선순위에 맞게 파싱을 재귀적으로 진행한다.(Parsing with precedence)
...
``` 
※ parsePrecedence　안의 while loop 에 의해서 infix 의 우측 피연산자가 어디까지 인가가 결정된다.  
...
1. -3 + 4 으로 해봄
2. * 포인터
3. `(a > b) ? a : b;` 와 같은 3항 연산자를 파싱 하려면?
  - `?`,`:` 토큰 추가, 
  - PREC_CONDITIONAL 를 Precedence 의 PREC_ASSIGNMENT 다음에 추가
  - 룰 추가
