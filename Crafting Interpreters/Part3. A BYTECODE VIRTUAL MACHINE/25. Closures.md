...
- CallFrame 안에 closure 가 있기 때문에, 다음과 같이 CallFrame 이 종료되면 closure 에 접근할 방법이 없다. 이 문제를 해겨하기 위해서 ObjUpvalue 를 링크드 리스트로 만들고, 
  최상위 vm 에서 openUpvalues 로 이것들을 관리하도록 한다.
```
fun outer() {
  var x = "outside";
  fun inner() {
