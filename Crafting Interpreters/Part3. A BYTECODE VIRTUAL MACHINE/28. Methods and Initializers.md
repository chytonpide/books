...
- JLox 와 Clox 모두 클래스의 표현에 메서드가 포함된다. JLox 에서 메서드 필드는 파싱의 결과 AST 형태로 존재하고 런타임에 바로 이것을 사용했지만, CLox 에서는 메서드 필드는 바이트코드 시퀸스로 런타임에 전달되어야 한다.    
...
- 록스에서 지역변수와 평가중의 임시값들을 스택에 있고, 전역 변수는 해시테이블, 클로저가 캡처한 바깥 스코프의 로컬 변수가 upvalue에 저장된다. method 리시버를 추적하기 위해서는 새로운 런타임 타입이 필요하고 그것이 ObjBoundMethod 이다.
