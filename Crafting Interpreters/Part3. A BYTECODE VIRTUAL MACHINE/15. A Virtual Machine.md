...
jlox 표현식을 평가할 때, 평과 과정에서 메서드내의 지역변수에 값을 저장한다. (메서드 호출마다 별도의 컨택스트를 가지고 호출이 끝나면 값들은 파기된다.)
평가된 값은 상위 노드로 이동하면서 다시 노드의 메서드내의 지역변수에 저장된다.
clox 에서 run 함수는 재귀적으로 동작하지 않는다. 실행 해야 하는 자료구조는 트리가 아닌 인스트럭션의 시리즈이고, 
여기서의 추적해야 하는 값의 행동(쓰고 읽는, 순서)은 stack LIFO 동작과 일치하기 때문에 stack 에 이 값들을 저장한다. 
...
- bytecode dispatch : opcode 를 확인 한 후에 instruction 의 시멘틱을 구현한 코드를 얻을 필요가 있다. 이 프로세스를 인스트럭션의 디코딩 또는 디스페칭(dispatching) 이라고 부른다.
```c
