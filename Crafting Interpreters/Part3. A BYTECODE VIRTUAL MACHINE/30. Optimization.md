...
해시테이블 프로로빙을 빠르게 만든다. 해시값을 계산하기 위해서 모듈려 연산을 사용하는데 이 연산에 시간이 많이 소모된다느 것을 프로파일러를 통해 발견한다.
해시값을 계산할 때 capacity 가 8로 시작해서 2배씩 증가한다는 사실을 활용하면,비트를 조작(bit masking)하는 걸로 모듈러 연산과 같은 결과를 얻을수 있음으로 이를 대치한다. 
...
- Bitmasking 은정수의 비트(0/1)를 “플래그 집합”처럼 써서, 여러 개의 ON/OFF 상태를 하나의 값에 압축해서 저장·조회·수정하는 기법.
![mask](mask.png)
