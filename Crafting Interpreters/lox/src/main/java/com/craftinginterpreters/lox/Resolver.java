...
public class Resolver implements Expr.Visitor<Void>, Stmt.Visitor<Void> {
  ...
  public Void visitVarStmt(Stmt.Var stmt) {
    ...
    if (stmt.initializer != null) {
      resolve(stmt.initializer); // var 을 initializer 에서 사용하려고 하면 define 되지 않은 상태여서 에러가 발생한다.
    }
    define(stmt.name);
    return null;
  }
  ...
  private void resolve(Stmt stmt) {
    stmt.accept(this); // Interpreter 의 execute 와 같은 역할
  }

  private void resolve(Expr expr) {
    expr.accept(this); // Interpreter 의 evaluate 와 같은 역할
  }
  ...
  private void endScope() {
    scopes.pop(); // resolved 된 정보가 interpreter 에 설정 된 후에, scope 는 사라진다.
  }
