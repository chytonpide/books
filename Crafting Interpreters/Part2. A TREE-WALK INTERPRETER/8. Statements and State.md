...
프로그램이 되려면 단순히 값을 처리하는것(표현식을 평가하는것) 뿐만 아니라 값을 유지하는 능력이 필요하다.
문장은 정의상 값을 반환하지 않음으로 이것이 할수 있는 유용한 것은 상태를 변경하는 것이다. 이를 사이드 이팩트라고도 부른다.
선언은 문장, 할당은 표현식이다.
...
※ Scope: 이름이 특정 엔티티에 맵핑되는 영역 → 규칙
※ Environment: 변수값을 저장하는 실제 데이터구조
※ Prod, Staging 에서의 Environment 도 "프로그램이 실행되는 컨텍스트" 라는 의미에서는 통하지만, 수준이 전혀 다르다.
