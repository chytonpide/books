...
static ObjUpvalue* captureUpvalue(Value* local) {
  ...
  if (prevUpvalue == NULL) {
    ...
    prevUpvalue->next = captureUpvalue;
  }
...
#define BINARY_OP(valueType, op) \
  ...
  printf("\n");
  disassembleInstruction(&frame->closure->function->chunk, (int)(frame->ip - frame->closure->function->chunk.code)); // &frame->closure->function->chunk 는 chunk 에 대한 포인터를 넘겨준다.
#endif
    uint8_t instruction;
    switch (instruction = READ_BYTE()) {
      ...
      case OP_GET_PROPERTY: {
        ...
        ObjString* name = READ_STRING(); // chunk.constants 에서 이름을 읽어온다.
      ...
      case OP_CLOSURE: {
        ...
        for (int i = 0; i < closure->upvalueCount; i++) {
          ...
          if (isLocal) {
            closure->upvalues[i] = captureUpvalue(frame->slots + index); // isLocal 인경우 위치를 기록한 ObjUpvalue 를 만든다.
          } else {
